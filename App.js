...
function renderizarFaixaAeroporto(lista) {
  ...
  // O conteúdo é duplicado (dois elementos idênticos lado a lado) para a
  // rolagem contínua em CSS (ver .faixa-aeroporto-trilho no styles.css).
  document.getElementById('conteudoFaixaAeroporto').textContent = texto;
  document.getElementById('conteudoFaixaAeroportoDuplicado').textContent = texto;

  // Ajusta a duração da animação proporcionalmente ao tamanho do texto, para
  // que a velocidade de rolagem pareça constante independente de quantos
  // indicadores estiverem sendo exibidos.
  const trilho = document.getElementById('trilhoFaixaAeroporto');
  if (trilho) {
    const duracaoSegundos = Math.max(12, texto.length * 0.16);
